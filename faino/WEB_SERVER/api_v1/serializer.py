...
from WEB_SERVER.models import Device, Data, Type, Command, Button, UserDevice
...
class Serializer_Device(serializers.ModelSerializer):
    ...
    def get_buttons(self, *args, **kwargs):
        this_device = args[0]
        buttons_records = Button.objects.filter(
            device=this_device, is_star=True)[0:5]
    ...
    def get_user_profile(self, *args, **kwargs):
        ...
        if user:
            this_device = args[0]
            userprofile = UserDevice.objects.get(
                device=this_device, user=user)
